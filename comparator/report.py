...
<head>
  ...
  <style>
    ...
    html[data-theme="light"] .theme-toggle:hover { background: #e2e8f0; }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .theme-toggle {
      ...
      color: var(--text-main); border-radius: 999px; padding: 11px 16px;
      font-weight: 700; font-size: 0.88rem; cursor: pointer;
      transition: transform 0.2s ease, background-color 0.2s ease, border-color 0.2s ease;
    }
    .theme-toggle:hover { transform: translateY(-1px); background: rgba(99, 102, 241, 0.1); }
    html[data-theme="light"] .theme-toggle { background: #eef2ff; }
...
<body>
  ...
  <script>
    ...
    function applyTheme(theme) {
      ...
      if (themeBtn) { themeBtn.textContent = theme === 'light' ? 'Switch to Dark' : 'Switch to Light'; }
            ...
            await html2pdf().set({
              ...
              html2canvas: {
                ...
                onclone: (clonedDoc) => {
                  ...
                  ['downloadPdfBtn', 'pdfStatus', 'themeToggleBtn'].forEach((id) => {
...
def _build_page(num: int, page: fitz.Page, records: list[dict],
    ...
    for name in CATEGORIES_LIST:
        ...
        if status == "FAIL":
            score -= CATEGORY_WEIGHTS.get(name, 0)
